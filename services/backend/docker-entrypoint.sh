...
if [ "$RUN_DB_MIGRATIONS_ON_STARTUP" != "true" ]; then
    echo "⏭️  RUN_DB_MIGRATIONS_ON_STARTUP=$RUN_DB_MIGRATIONS_ON_STARTUP - skipping startup migrations"
    ...
    for attempt in $(seq 1 "$MIGRATION_MAX_RETRIES"); do
        ...
        if [ "$attempt" -lt "$MIGRATION_MAX_RETRIES" ]; then
            echo "⚠️  Migration attempt failed, retrying in ${MIGRATION_RETRY_DELAY_SECONDS}s..."
